...
export function InternalEditor({ documentId, initialStatus, initialPayload }: Props) {
  ...
  const { persist, markDirty, persistStatus, message, messageTone, setMessage } =
    useDocumentPersistence(() =>
      saveInternalDocumentAction({ id: documentId, status, payload }),
    );
  ...
  return (
    <OfficialEditorFrame
      title={title}
      typeLabel="ประเภท: หนังสือภายใน (บันทึกข้อความ)"
      formTitle="กรอกข้อมูลบันทึกข้อความ"
      message={message}
      ...
      preview={<InternalLetterPreview data={payload} />}
      zoomPreview={<InternalLetterPreview data={payload} printMode />}
    />
  );
}
